...
(In the development environment, we created policies to be able to create profile, post and comment from Vercel.)
...
-- ALLOW SELECT TO ANYONE (ONLY TO READ FROM VERCEL)
CREATE POLICY "READ POSTS FOR EVERYONE"
ON POSTS
FOR SELECT
USING ( TRUE );

-- ALLOW INSERT POST EVEN FROM ANON (API WITH ANON KEY - ONLY IN TO INSERT FROM VERCEL)
...
-- ALLOW INSERT COMMENTS EVEN FROM ANON (API WITH ANON KEY - ONLY TO INSERT FROM VERCEL)
CREATE POLICY "INSERT COMMENTS FROM API"
ON COMMENTS
FOR INSERT
TO ANON
WITH CHECK ( TRUE );

-- ALLOW EACH USER TO CREATE THEIR OWN PROFILE (ONLY TO INSERT FROM VERCEL)
...
-- ALLOW EACH USER TO UPDATE THEIR OWN PROFILE (ONLY TO UPDATE FROM VERCEL)
